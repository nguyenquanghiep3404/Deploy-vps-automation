...
- **Quality gate before deployment 🆕**: For Node.js/SPA projects, the tool detects `lint`, `typecheck`, and `test` scripts in `package.json` and runs them before building and deploying. A failing script stops the workflow before rsync; missing scripts are skipped. For workspaces/Turbo, root scripts take priority.
