...
- **Phiên bản Node 🆕**: Đọc `engines.node` (trong `package.json`) hoặc file `.nvmrc` để dùng đúng phiên bản Node trên cả runner và VPS, thay vì cố định Node 22 (vẫn có sàn tối thiểu Node 20 LTS trên VPS).
...
- **Build trên Runner (không build trên VPS) 🆕**: Với app Node có script `build` (vd backend TypeScript `tsc` → `dist/server.js`, hoặc Next.js), tool chạy `npm run build` **trên GitHub Runner** rồi rsync kết quả (`dist/`, `.next/`) sang VPS, sau đó mới `pm2 restart`. Nhờ vậy `dist/` **không cần nằm trong git** (được build mới mỗi lần push) và VPS không bị quá tải khi build. Với Monorepo **workspace mà chỉ package con có script build** (gốc không có), tool tự build **đúng trong package con** để không bị thiếu `dist/`.
- **Quality gate trước khi deploy 🆕**: Với Node.js/SPA, tool tự dò các script `lint`, `typecheck`, `test` trong `package.json` và chạy chúng trước build/deploy. Script nào lỗi thì workflow dừng, không rsync code lỗi lên VPS; script không có sẽ tự bỏ qua. Với workspace/Turbo, tool ưu tiên script tại gốc repo.
...
> 🔁 **Cổng ổn định khi deploy lại**: Khi bạn chạy lại tool cho cùng một domain, tool đọc cổng đã gán trong cấu hình Nginx cũ và **tái sử dụng đúng cổng đó** — không "trôi" sang cổng mới mỗi lần, không bỏ lại process PM2 cũ.
